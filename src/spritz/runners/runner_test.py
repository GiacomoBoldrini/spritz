        ...
        for dataset_name in results:
            for region in regions:
                ...
                for variable in results[dataset_name]["histos"]:
                    unw = False if "unweighted" not in variables[variable] else variables[variable]["unweighted"]

                    if unw: 
                        print("RICHIESTO UNWEIGHTED")
                        evweight = ak.ones_like(events["weight"][mask])
                    else: 
                        evweight = events["weight"][mask]
