...
print(corr["description"], corr["corrections"][0].keys(),corr["corrections"][0]["data"].keys())
...
def get_cset_electron(era, wp, return_histo=False):
    ...
    for valType in ["sf", "sfdown", "sfup"]:
        ...
        obj = sf[wp_ind]["value"]
        # if obj["edges"][0][0] == -np.inf:
        #     obj["edges"][0][0] = -10000000000000.0
        # if obj["edges"][0][-1] == np.inf:
        #     obj["edges"][0][-1] = +10000000000000.0
        # if obj["edges"][1][0] == -np.inf:
        #     obj["edges"][1][0] = -10000000000000.0
        # if obj["edges"][1][-1] == np.inf:
        #     obj["edges"][1][-1] = +10000000000000.0
    ...
    #if wp in ["RecoBelow20", "RecoAbove20"]: syst = ["nominal", "syst_down", "syst_up"]
    #elif wp == "wp90iso": syst = ["nominal", "stat", "syst"]
    # syst = ['sf', 'sfdown', 'sfup']
    h = hist.Hist(
    ...
    if wp in ["RecoBelow20", "RecoAbove20"]: name = f"Electron_RecoSF_" + wp
    elif wp == "wp90iso": name = "Electron_WP_SF"
    h.name = name
    h.label = "out"
    print(h)
...
for wp in ["RecoBelow20", "RecoAbove20", "wp90iso"]:
    print(f"-----> {wp}")
